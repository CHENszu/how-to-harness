...
WORKDIR = Path.cwd()
client = OpenAI(api_key=os.getenv("ANTHROPIC_API_KEY"), base_url=os.getenv("ANTHROPIC_BASE_URL", "https://api.deepseek.com/v1"))
...
def spawn_subagent(description: str) -> str:
    ...
    for _ in range(30):
        call_msgs = [{"role": "system", "content": SUB_SYSTEM}] + messages
        try:
            response = client.chat.completions.create(
                model=MODEL, messages=call_msgs,
                tools=SUB_TOOLS, max_tokens=8000)
...
def agent_loop(messages: list, context: dict):
    ...
    while True:
        ...
        # Add background notifications if any
        inject_background_notifications(messages)
...
def print_turn_assistants(messages: list, turn_start: int):
    for msg in messages[turn_start:]:
        if msg.get("role") != "assistant":
            continue
        if msg.get("content"):
            terminal_print(str(msg["content"]))
