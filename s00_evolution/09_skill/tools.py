...
class SkillsListTool(BaseTool):
    name = "skills_list"
    description = "列出所有可用的按需加载技能（Skills）。返回技能名称和简短描述。当你需要了解自己拥有哪些扩展能力时使用此工具。"
...
# ==========================================
# 工具 6：查看技能详情 (SkillViewTool)
# ==========================================
class SkillViewToolInput(BaseModel):
    name: str = Field(description="需要查看的技能名称")

class SkillViewTool(BaseTool):
    name = "skill_view"
    description = "查看特定技能的详细说明和指令。当你需要执行某项任务而该任务匹配某个可用技能时，必须先使用此工具加载该技能的详细指令。"
    ...
    def execute(self, **kwargs) -> str:
        ...
        print(f"  [SkillView] 正在加载技能详情: `{name}`")
