...
# 核心循环 (Agent Loop)
# ==========================================
def run_agent_loop(user_input: str, registry: ToolRegistry, messages: list, hook_executor: HookExecutor = None) -> list:
    print(f"\n[用户]: {user_input}")
    ...
    while turn <= MAX_TURNS:
        ...
        try:
            ...
            if message.tool_calls:
                for tool_call in message.tool_calls:
                    ...
                    if target_tool:
                        try:
                            ...
                            # --- Hook 卡点：PRE_TOOL_USE ---
                            if hook_executor:
                                pre_hooks = hook_executor.execute(
                                    HookEvent.PRE_TOOL_USE,
                                    {"tool_name": func_name, "tool_input": args_str}
                                )
                        ...
                        # --- Hook 卡点：POST_TOOL_USE ---
                        if hook_executor:
                            post_hooks = hook_executor.execute(
                                HookEvent.POST_TOOL_USE,
                                {"tool_name": func_name, "tool_result": str(result)[:500]}
                            )
                            # POST_TOOL_USE 一般用于观察，这里假设即使拦截也不阻止结果返回，或者把错误附加进去
                            if post_hooks.blocked:
                                result = f"{result}\n\n[Hook警告]: {post_hooks.error}"
                        # ---------------------------------
                    else:
                ...
                # --- Hook 卡点：BEFORE_REPLY ---
                if hook_executor:
                    ctx = {"user_input": user_input, "final_message": final_msg}
                    hook_executor.execute(HookEvent.BEFORE_REPLY, ctx)
                    final_msg = ctx["final_message"]
                # -------------------------------
                ...
                # --- Hook 卡点：STOP ---
                if hook_executor:
                    hook_executor.execute(HookEvent.STOP, {"final_message": final_msg})
                # -----------------------
                break
