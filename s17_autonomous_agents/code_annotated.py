            ...
            while True:
                ...
                for _ in range(10):
                    ...
                    for tool_call in response_message.tool_calls:
                        ...
                        if handler:
                            try:
                                output = handler(**block_input)
                            except Exception as e:
                                output = f"Error executing tool {block_name}: {str(e)}. Please check your arguments and try again."
                        else:
                            output = f"Unknown tool: {block_name}"
...
TOOLS = [
    ...
    {"name": "create_task", "description": "Create a task.",
     "parameters": {"type": "object", "properties": {"subject": {"type": "string"}, "description": {"type": "string"}, "blockedBy": {"type": "array", "items": {"type": "string"}}}, "required": ["subject"]}},
...
def agent_loop(messages: list, context: dict, tools: list[dict] | None = None,
               role_name: str = "lead"):
    if tools is None:
        tools = TOOLS
    ...
    while True:
        ...
        for tool_call in response_message.tool_calls:
            ...
            print(f"\033[36m> {block_name}\033[0m")
...
if __name__ == "__main__":
    print("s17: Autonomous Agents — 自己看板，自己认领 (中文版)")
    print("试着输入：'创建 3 个互相独立的任务，然后召唤两个队友 alice 和 bob。看着他们自己认领任务干活。'\n")
    history = []
    context = {"memories": ""}
    had_teammates = False
    while True:
        ...
        if active:
            had_teammates = True
        elif had_teammates and not active:
