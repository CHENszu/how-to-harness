...
def spawn_teammate_thread(name: str, role: str, prompt: str) -> str:
    ...
    system = (f"You are '{name}', a {role}. "
              ...
              f"Send results via send_message to 'lead'. Act, don't explain.")
    ...
    def run():
        try:
            ...
            print(f"  \033[35m[Team] 队友 {name} 退出\033[0m")
        except Exception as e:
...
def agent_loop(messages: list, context: dict, tools: list[dict],
    ...
    while True:
        # Check inbox and inject
        inbox = BUS.read_inbox(role_name)
        if inbox:
            inbox_text = "\n".join(
                f"From {m['from']}: {m['content'][:200]}" for m in inbox)
            messages.append({"role": "user",
                             "content": f"[Inbox]\n{inbox_text}"})
            print(f"  \033[32m[Inbox] 老大收到 {len(inbox)} 条新消息并触发重试\033[0m")
        ...
        if not response_message.tool_calls:
            print("  \033[90m[Agent 空闲。等待新指令 或 定时任务触发...]\033[0m")
            while True:
                time.sleep(0.5)
                ...
                if fired_jobs:
                    for job in fired_jobs:
                        ...
                        print(f"  \033[35m[定时触发] 任务 {job.id}: {job.prompt}\033[0m")
                    break
                    
                # 3. Check inbox
                inbox_check = BUS.read_inbox(role_name)
                if inbox_check:
                    inbox_text = "\n".join(
                        f"From {m['from']}: {m['content'][:200]}" for m in inbox_check)
                    messages.append({"role": "user",
                                     "content": f"[Inbox]\n{inbox_text}"})
                    if role_name == "lead":
                        print(f"  \033[32m[Inbox] 老大收到 {len(inbox_check)} 条新消息并触发重试\033[0m")
                    break
            
            if not bg_notifications and not fired_jobs and not inbox_check:
                return
            continue
...
if __name__ == "__main__":
    print("s15: Agent Teams — 团队协作 (中文版)")
    print("试着输入：'请召唤一个名为 alice 的后端开发队友，让她创建一个 schema.sql 文件'\n")
    history = []
