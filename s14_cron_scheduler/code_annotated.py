...
def agent_loop(messages: list, context: dict) -> dict:
    ...
    while True:
        ...
        if not response_message.tool_calls:
            ...
            if not bg_notifications and not fired_jobs:
                return context
            continue
...
def print_latest_assistant_text(messages: list):
    ...
    if isinstance(content, str):
        # 已经在 agent_loop 中打印过了，这里不需要重复打印
        return
...
def queue_processor_loop():
    ...
    while True:
        ...
        if not has_cron_queue():
            continue
        # 尝试获取锁，如果拿不到（说明 Agent 正在忙），就跳过等下一轮
        if not agent_lock.acquire(blocking=False):
            continue
        try:
            if not has_cron_queue():
                continue
            print("\n  \033[35m[Queue Processor] 发现队列中有定时任务，拉起新一轮 Agent 执行...\033[0m")
...
if __name__ == "__main__":
    print("s14: Cron Scheduler — 定时任务调度器 (中文版)")
    print("试着输入：'创建一个每 1 分钟执行一次的定时任务，打印当前时间'\n")
    threading.Thread(target=queue_processor_loop, daemon=True).start()
    print("  \033[35m[Queue Processor] 秘书线程已启动，时刻盯防队列\033[0m")
    while True:
        try:
